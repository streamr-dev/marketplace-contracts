...
contract("Marketplace", accounts => {
    ...
    describe("Subscription", () => {
        ...
        it("can be extended", async () => {            
            const [valid_before, endtime_before] = await market.getSubscriptionTo("test_sub", {from: accounts[1]})
            assert(valid_before)
            await market.buy("test_sub", 100, {from: accounts[1]})
            const [valid_after, endtime_after] = await market.getSubscriptionTo("test_sub", {from: accounts[1]})
            assert(valid_after)            
            assert(endtime_after - endtime_before > 100 - testToleranceSeconds)
        })

        it("can be transferred", async () => {
            const [valid1_before, endtime1_before] = await market.getSubscription("test_sub", accounts[1])
            const [valid2_before, endtime2_before] = await market.getSubscription("test_sub", accounts[2])
            await market.transferSubscription("test_sub", accounts[2], {from: accounts[1]})
            const [valid1_after, endtime1_after] = await market.getSubscription("test_sub", accounts[1])
            const [valid2_after, endtime2_after] = await market.getSubscription("test_sub", accounts[2])
