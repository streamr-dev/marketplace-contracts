...
const MarketplaceJsonPrev = require('../build/contracts/Marketplace20180425.json')
...
describe('Marketplace', () => {
    it('can createProduct and buy also outside Truffle', async () => {
        const MarketplacePrev = new w3.eth.Contract(MarketplaceJsonPrev.abi)
        ...
        const marketPrev = await MarketplacePrev
            .deploy({
                data: MarketplaceJsonPrev.bytecode, arguments: [token.options.address, accounts[8]]
            })
            .send({
        ...
        const market = await Marketplace
            .deploy({
                data: MarketplaceJson.bytecode,
                arguments: [token.options.address, accounts[8], marketPrev.options.address]
            })
            .send({
                from: accounts[0], gas: 6000000
            })
        ...
        await token.methods.mint(accounts[1], 100000).send({
            from: accounts[0], gas: 4000000
        })
        await token.methods.approve(market.options.address, 10000).send({
